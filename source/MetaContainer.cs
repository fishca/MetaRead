                    ...
                    switch (t.Serialization_Ver)
                    {
                        ...
                        case 3:
                            ...
                            else if (tr.Get_Type() == Node_Type.nd_list)
                            {
                                ...
                                if (tt is null)
                                {
                                    Form1.log.Error($"Ошибка формата потока 70. Ожидается значение системного перечисления. " +
                                                    $"Загружаемый тип {t.Name}, " +
                                                    $"Путь {spath + tr.Path()}");
                                    break;
                                }
                                ...
                                {
                                    Form1.log.Error($"Ошибка формата потока 71. Тип значения не Число, ожидается значение системного перечисления. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Тип значения {tt.Get_Type()}" +
                                                    $"Путь {spath + tt.Path()}");
                                    break;
                                ...
                                if (v is null)
                                {
                                    Form1.log.Error($"Ошибка формата потока 72. Не найдено значение системного перечисления по числовому значению. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Значение {i}" +
                                                    $"Путь {spath + tt.Path()}");
                                }
                        ...
                        case 4:
                            ...
                            if (tr.Get_Type() != Node_Type.nd_guid)
                            {
                                Form1.log.Error($"Ошибка формата потока 100. Тип значения не UID. " +
                                                $"Загружаемый тип {t.Name}" +
                                                $"Тип значения {tr.Get_Type()}" +
                                                $"Путь {spath + tr.Path()}");
                            }
                            else
                            {
                                if (!string_to_GUID(tr.Get_Value(), ref uid))
                                {
                                    Form1.log.Error($"Ошибка формата потока 101. Ошибка преобразвания UID. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Значение {tr.Get_Value()}" +
                                                    $"Путь {spath + tr.Path()}");
                                }
                        ...
                        case 5: // Стандартный реквизит
                            ...
                            if (tr.Get_Type() != Node_Type.nd_number)
                            {
                                Form1.log.Error($"Ошибка формата потока 104. Тип значения не Число при загрузке стандартного реквизита. " +
                                                $"Загружаемый тип {t.Name}" +
                                                $"Тип значения {tr.Get_Type()}" +
                                                $"Значение {tr.Get_Value()}" +
                                                $"Путь {spath + tr.Path()}");
                            ...
                            else
                            {
                                ...
                                if (metauid == EmptyUID)
                                {
                                    Form1.log.Error($"Ошибка формата потока 105. Ошибка загрузки стандартного реквизита. Не определён metauid. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Значение {tr.Get_Value()}" +
                                                    $"Путь {spath + tr.Path()}");
                                }
                                else
                                {
                                    ...
                                    if (tr != null && i >= 0)
                                    {
                                        ...
                                        {
                                            Form1.log.Error($"Ошибка формата потока 205. Ошибка загрузки стандартного реквизита. Тип значения не UID. " +
                                                            $"Загружаемый тип {t.Name}" +
                                                            $"Тип значения {tr.Get_Type()}" +
                                                            $"Значение {tr.Get_Value()}" +
                                                            $"Путь {spath + tr.Path()}");
                        ...
                        case 6:
                            ...
                            if (tr.Get_Type() != Node_Type.nd_string)
                            {
                                Form1.log.Error($"Ошибка формата потока 111. Тип значения не Строка. " +
                                                $"Загружаемый тип {t.Name}" +
                                                $"Тип значения {tr.Get_Type()}" +
                                                $"Значение {tr.Get_Value()}" +
                                                $"Путь {spath + tr.Path()}");
                            ...
                            else
                            {
                                ...
                                {
                                    Form1.log.Error($"Ошибка формата потока 112. Неизвестное значение частей даты. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Значение {s}" +
                                                    $"Путь {spath + tr.Path()}");
                                }
                        ...
                        case 7: // Стандартная табличная часть
                            ...
                            if (tr.Get_Type() == Node_Type.nd_number)
                            {
                                Form1.log.Error($"Ошибка формата потока 207. Тип значения не Число при загрузке стандартной табличной части. " +
                                                $"Загружаемый тип {t.Name}" +
                                                $"Тип значения {tr.Get_Type()}" +
                                                $"Значение {tr.Get_Value()}" +
                                                $"Путь {spath + tr.Path()}");
                            ...
                            else
                            {
                                ...
                                if (metauid == EmptyUID)
                                {
                                    Form1.log.Error($"Ошибка формата потока 208. Ошибка загрузки стандартной табличной части. Не определён metauid. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Значение {tr.Get_Value()}" +
                                                    $"Путь {spath + tr.Path()}");
                                }
                        ...
                        case 8:
                            ...
                            if (tr.Get_Type() != Node_Type.nd_list)
                            {
                                Form1.log.Error($"Ошибка формата потока 216. Ошибка загрузки типа МетаСсылка. Узел не является списком. " +
                                                $"Загружаемый тип {t.Name}" +
                                                $"Тип значения {tr.Get_Type()}" +
                                                $"Значение {tr.Get_Value()}" +
                                                $"Путь {spath + tr.Path()}");
                            ...
                            else
                            {
                                ...
                                if (tt is null)
                                {
                                    Form1.log.Error($"Ошибка формата потока 217. Ошибка загрузки типа МетаСсылка. Нет значений. " +
                                                    $"Загружаемый тип {t.Name}" +
                                                    $"Путь {spath + tt.Path()}");
                                }
                                else
                                {
                                    if (tt.Get_Type() != Node_Type.nd_number)
                                    {
                                        Form1.log.Error($"Ошибка формата потока 218. Ошибка загрузки типа МетаСсылка. Узел не является числом. " +
                                                        $"Загружаемый тип {t.Name}" +
                                                        $"Тип значения {tt.Get_Type()}" +
                                                        $"Значение {tt.Get_Value()}" +
                                                        $"Путь {spath + tt.Path()}");
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            Form1.log.Error($"Ошибка формата потока 219 Ошибка загрузки типа МетаСсылка. Значение узла не равно 1. " +
                                                            $"Загружаемый тип {t.Name}" +
                                                            $"Значение {tt.Get_Value()}" +
                                                            $"Путь {spath + tt.Path()}");
                                        }
                                        ...
                                        if (tt.Get_Type() != Node_Type.nd_guid)
                                        {
                                            Form1.log.Error($"Ошибка формата потока 220. Ошибка загрузки типа МетаСсылка. Узел не UID. " +
                                                            $"Загружаемый тип {t.Name}" +
                                                            $"Тип значения {tt.Get_Type()}" +
                                                            $"Значение {tt.Get_Value()}" +
                                                            $"Путь {spath + tt.Path()}");
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Form1.log.Error($"Ошибка формата потока 221. Ошибка преобразования UID при загрузке ссылки на объект метаданных. " +
                                                                $"Загружаемый тип {t.Name}" +
                                                                $"Значение {tt.Get_Value()}" +
                                                                $"Путь {spath + tt.Path()}");
                                            }
                                            ...
                                            if (tt.Get_Type() != Node_Type.nd_number)
                                            {
                                                Form1.log.Error($"Ошибка формата потока 222. Ошибка загрузки типа МетаСсылка. Узел не является числом. " +
                                                                $"Загружаемый тип {t.Name}" +
                                                                $"Тип значения {tt.Get_Type()}" +
                                                                $"Значение {tt.Get_Value()}" +
                                                                $"Путь {spath + tt.Path()}");
                                            ...
                                            else
                                            {
                                                ...
                                                for (i = 0; i < k; ++i)
                                                {
                                                    ...
                                                    {
                                                        Form1.log.Error($"Ошибка формата потока 225. Ошибка загрузки типа МетаСсылка. Узел не является списком. " +
                                                                        $"Загружаемый тип {t.Name}" +
                                                                        $"Тип значения {tt.Get_Type()}" +
                                                                        $"Значение {tt.Get_Value()}" +
                                                                        $"Путь {spath + tt.Path()}");
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        if (ttt is null)
                                                        {
                                                            Form1.log.Error($"Ошибка формата потока 226. Ошибка загрузки типа МетаСсылка. Нет значений. " +
                                                                            $"Загружаемый тип {t.Name}" +
                                                                            $"Путь {spath + tt.Path()}");
                                                        }
                                                        else
                                                        {
                                                            ...
                                                            if (ttt is null)
                                                            {
                                                                Form1.log.Error($"Ошибка формата потока 227. Ошибка загрузки типа МетаСсылка. Нет значений. " +
                                                                                $"Загружаемый тип {t.Name}" +
                                                                                $"Путь {spath + tx.Path()}");
                                                            }
                                                            else
                                                            {
                                                                ...
                                                                {
                                                                    Form1.log.Error($"Ошибка формата потока 228. Ошибка загрузки типа МетаСсылка. Узел не UID. " +
                                                                                    $"Загружаемый тип {t.Name}" +
                                                                                    $"Тип значения {ttt.Get_Type()}" +
                                                                                    $"Значение {ttt.Get_Value()}" +
                                                                                    $"Путь {spath + ttt.Path()}");
                                                                }
                                                                else
                                                                {
                                                                    ...
                                                                    {
                                                                        Form1.log.Error($"Ошибка формата потока 229. Ошибка преобразования UID при загрузке типа МетаСсылка. " +
                                                                                        $"Загружаемый тип {t.Name}" +
                                                                                        $"Тип значения {ttt.Get_Type()}" +
                                                                                        $"Значение {ttt.Get_Value()}" +
                                                                                        $"Путь {spath + ttt.Path()}");
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        {
                                                                            Form1.log.Error($"Ошибка формата потока 230. Неизвестная сигнатура при загрузке типа МетаСсылка. " +
                                                                                            $"Загружаемый тип {t.Name}" +
                                                                                            $"Значение {ttt.Get_Value()}" +
                                                                                            $"Путь {spath + ttt.Path()}");
                                                                        }
                                                ...
                                                if (tt.Get_Type() != Node_Type.nd_number)
                                                {
                                                    Form1.log.Error($"Ошибка формата потока 223. Ошибка загрузки типа МетаСсылка. Узел не является числом. " +
                                                                    $"Загружаемый тип {t.Name}" +
                                                                    $"Тип значения {tt.Get_Type()}" +
                                                                    $"Значение {tt.Get_Value()}" +
                                                                    $"Путь {spath + tt.Path()}");
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        Form1.log.Error($"Ошибка формата потока 224. Ошибка загрузки типа Булево. " +
                                                                        $"Загружаемый тип {t.Name}" +
                                                                        $"Значение {tt.Get_Value()}" +
                                                                        $"Путь {spath + tt.Path()}");
                                                        b = false;
                        ...
                        default:
                            ...
                            Form1.log.Error($"Ошибка формата потока 16. Неизвестный вариант сериализации. " +
                                            $"Загружаемый тип {t.Name}" +
                                            $"Вариант сериализации {t.Serialization_Ver.ToString()}" +
                                            $"Путь {spath + tr.Path()}");
                            ptr = tr.Get_Next();
                            break;
                    }
